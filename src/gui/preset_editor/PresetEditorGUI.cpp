...
void PresetEditorGUI::Show(const std::string& presetFile)
{
    ...
    else
    {
        ...
        {
            Poco::NotificationCenter::defaultCenter().postNotification(new Notification::DisplayToast("The selected preset could not be loaded."));
            return;
        }
    }
    ...
    {
        Poco::NotificationCenter::defaultCenter().postNotification(new Notification::DisplayToast(errorMessage));
...
void PresetEditorGUI::UpdatePresetPreview()
{
    ...
    if (!_projectMWrapper.LoadPresetData(_presetFile.AsString(), errorMessage))
    {
        Poco::NotificationCenter::defaultCenter().postNotification(new Notification::DisplayToast("Preset reload failed:\n" + errorMessage));
    }
}
...
void PresetEditorGUI::TakeProjectMControl()
{
    ...
    Poco::NotificationCenter::defaultCenter().postNotification(new Notification::UpdateWindowTitle("projectM Preset Editor"));
}
